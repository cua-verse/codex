...
pub(crate) fn build_prompt(
    ...
    // OpenRouter doesn't accept the Responses-API namespaced-tool grouping, so
    // flatten MCP namespaces into plain `mcp__<server>__<tool>` function tools
    // (and fill empty parameter schemas) when routing through it. No-op for the
    // default OpenAI provider. (agenthle: re-integrated onto upstream's
    // simplified build_prompt — deferred-dynamic filtering now lives in the
    // router's model_visible_specs().)
